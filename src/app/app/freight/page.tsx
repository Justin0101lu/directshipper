  ...
  return (
    <>
      ...
      {hasLoads && p && (<>
        ...
        {(me?.authorities.length ?? 0) > 0 && <div className="panel"><h3>Your companies</h3><p className="ph">The carrier named on each rate con, grouped by MC. A rate con tendered to a company you do not own means the reader guessed; rename it under Settings &rarr; Sources.</p>
          <table style={{ border: "none" }}><thead><tr><th>Company</th><th>MC</th><th className="right">Loads</th></tr></thead><tbody>{me!.authorities.map((a) => <tr key={a.id} style={{ cursor: "default" }}><td className="lead">{a.name}</td><td data-label="MC" className="num">{a.mc || "\u2014"}</td><td data-label="Loads" className="num right">{a.loads}</td></tr>)}</tbody></table></div>}
