export type Company = { name: string; website: string | null; city: string | null; state: string | null; industry: string | null; employees: number | null };
...
export type Provider = {
  ...
  findPeople?: (company: string, domain: string | null, titles: string[], size: number, near?: { city: string; stateName: string }) => Promise<Person[] | null>;
  /* Companies in an industry and state, from a company database (cold prospects). */
  findCompanies?: (industries: string[], stateName: string, size: number) => Promise<Company[] | null>;
