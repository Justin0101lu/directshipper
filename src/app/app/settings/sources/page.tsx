...
export default function Sources() {
  ...
  return (
    <>
      ...
      {me && (
        <div className="panel"><h3>Sending limits</h3><p className="ph">The pace that keeps a cold inbox out of spam. Limits apply to each mailbox. Scale volume by adding mailboxes, not raising caps.</p>
          <table style={{ border: "none", maxWidth: 640 }}><tbody>
            ...
            <tr style={{ cursor: "default" }}><td className="lead">Calls per day</td><td className="small">Each sequence has one call, on day 3, with a script written from your history. This is how many land on your list in a day.</td><td className="right"><input type="number" min={0} max={40} defaultValue={me.sending.callsPerDay} onBlur={(e) => Number(e.target.value) !== me.sending.callsPerDay && setSending({ callsPerDay: Number(e.target.value) })} style={{ width: 72, padding: "6px 8px", fontSize: 14 }} /></td></tr>
          </tbody></table></div>)}
