...
LinkedIn steps are copy-and-paste until a person connects their LinkedIn under Settings → Sources (the LinkedIn column next to their mailbox). The connection runs through Unipile's hosted login page, so no LinkedIn password touches this app; set `UNIPILE_DSN`, `UNIPILE_API_KEY` and `UNIPILE_WEBHOOK_SECRET`. Once connected, `sendLinkedinQueued` in `src/lib/outreach` sends connection requests (note cut at 200 characters) and messages on that account, one action at a time with a 5 to 10 minute gap, capped at 10 invites and 20 messages a day. Already-connected people skip the invite. A message to someone who has not accepted yet falls back to the copy list rather than InMail. A LinkedIn reply arrives on the messaging webhook and stops the sequence like an email reply. LinkedIn's user agreement forbids automation; the caps are what keep accounts out of restriction, and the app never raises them on its own.
